...
function FamilyPDF(){
    ...
    imgSrc={value.entity.img}
     ...
     rupees={`Amount: ${value.totalAmount}`}
 />)
    return(
        <Document>
        <Page size="A4" style={styles.box} wrap>
          ...
          <Head />
          <CustInfo name={data.fullName} heading={"Customer Details"} email={data.email} age={`User Age: ${data.userAge}`} sAge={data.spouseAge} cAge={data.children_Age?.toString()} phone={data.phone} hospLimit={data.insurancePrize}/>
